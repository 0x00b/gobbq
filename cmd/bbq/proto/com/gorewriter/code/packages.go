...
// LoadWithTypes tries a standard load, which may not have enough type info (TypesInfo== nil) available
// if the imported package is a second order dependency. Fortunately this doesnt happen very often,
// so we can just issue a load when we detect it.
