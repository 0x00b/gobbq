...
func parseFile(inputPath string, pb *com.File, src any, xxxSkip []string) (areas []textArea, err error) {
	...
	for _, decl := range f.Decls {
		...
		for _, m := range pb.Messages {
			if m.GoIdent.GoName == typeSpec.Name.Name {
				msg = m
				break
			}
		}
		...
		for _, field := range structDecl.Fields.List {
			...
			tag := ""
			for _, pbfield := range msg.Fields {
				if pbfield.GoName == field.Names[0].Name {
					tag = FieldTag(pbfield)
					break
				}
			}
			if tag == "" {
